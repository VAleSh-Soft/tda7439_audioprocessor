...
Модуль позволяет выбирать вход и настраивать параметры звука. Параметры тембра, баланса, приглушения и предусиления сохраняются для каждого входного канала в **EEPROM**.
...
*Питание Bt-модуля включается при первом переходе на вход, к которому он подключен; при переходе на другой вход питание Bt-модуля остается включенным, чтобы иметь возможность переключать входы TDA7439 без потери сигнала*
