...
Основные технические характеристики ИМС TDA7439 в этом модуле (некоторые параметры программно ограничены):
...
- Предусиление от 0 до 30 дБ (с шагом 2 дБ, 0..14), независимое для каждого канала;
- Приглушение звука для входа от -14 до 0 дБ (ограничено программно);
  (*предусиление и приглушение регулируются одновременно в пределах -14..14*)
...
Модуль позволяет выбирать вход и настраивать параметры звука. Параметры громкости, тембра, баланса, приглушения и предусиления сохраняются для каждого входного канала в **EEPROM**.
...
Модуль выводит данные на текстовый **LCD1602**-экран. Кроме того используется светодиод (красный) для индикации режима **MUTE** (мигает, если звук отключен), а так же, если ипользуется Bluetooth трансмиттер, светодиод для индикации его работы (синий, включается, если выбран канал с трансмиттером).

#### Управление

По умолчанию модуль управляется одним энкодером со встроенной кнопкой:

***Важно!!!** - при необходимости модуль позволяет подключить и использовать дополнительные кнопки - кнопку быстрого отключения звука, кнопку выбора режима экрана и кнопку переключения текущего входа*
...
- клик кнопкой энкодера перебирает параметры для регулировки:
  ...
  - тембр ВЧ (**treble**);
  - баланс (**Balance**);
- двойной клик кнопкой энкодера переводит модуль в режим настройки уровня предусиления входа (**InputGain**); позволяет как усилить, так и ослабить уровень звука канала;
...
Внесенные изменения в настройки параметров сохраняются через две секунды после последней манипуляции энкодером/кнопками. После десяти секунд отсутствия активности пользователя модуль переходит в режим по умолчанию (регулировка громкости). Экран при этом гаснет (если задана опция `TURN_OFF_SCREEN_BACKLIGHT`);

Если экран отключен, любая манипуляция энкодером/кнопками сначала включает его, не изменяя настройки.
...
***Важно!!!** - питание Bt-модуля включается при первом переходе на вход, к которому он подключен; при переходе на другой вход питание Bt-модуля остается включенным, чтобы иметь возможность переключать входы TDA7439 без потери сигнала*

`#define USE_DEBUG_OUT 0` - включить вывод отладочной информации через интерфейс UART; 1 - включить, 0 - отключить;

`#define DEBUG_BAUD_COUNT 115200` - скорость работы UART-интерфейса;

`#define USE_MUTE_BUTTON 0` - использовать дополнительную кнопку для управления отключением звука; клик этой кнопкой дублирует удержание нажатой кнопки энкодера; 1 - использовать кнопку, 0 - не использовать;

`#define USE_MODE_BUTTON 0` - использовать кнопку для переключения режима управления - что в данный момент регулирует крутилка энкодера; клик и двойной клик дублируют таковые действия кнопки энкодера; 1 - использовать кнопку, 0 - не использовать;

`#define USE_INPUT_BUTTON 0` - использовать кнопку для переключения текущего входа; переключение идет по кругу от меньшего к большему; 1 - использовать кнопку, 0 - не использовать;
...
`constexpr uint32_t TIMEOUT_OF_AUTOSAVE_DATA = 2;` - таймаут задержки сохранения параметров в **EEPROM**, в секундах;
...
`constexpr uint8_t VOLTAGE_CONTROL_PIN = 2;` - пин для контроля пропадания напряжения питания; используется для включения режима **MUTE** при отключении питания, чтобы избежать щелчка в колонках; так же блокируется запись в EEPROM для исключения порчи данных; для Atmega168/328 можно использовать пины 2 или 3; примерная схема подключения:
