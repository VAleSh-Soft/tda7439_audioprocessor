...
`constexpr uint8_t VOLTAGE_CONTROL_PIN = 2;` - пин для контроля пропадания напряжения питания; используется для включения режима **MUTE** при отключении питания, чтобы избежать щелчка в колонках; можно использовать значение 2 или 3; примерная схема подключения:
